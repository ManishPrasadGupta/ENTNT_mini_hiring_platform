...
export default function JobDetailsPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-emerald-50 to-emerald-100 px-2 md:px-0 transition-all duration-500">
      <div className="w-full max-w-xl bg-white/90 p-8 md:p-10 rounded-2xl shadow-2xl transition-all duration-500 animate-fade-in">
        <button
          onClick={() => router.back()}
          className="mb-7 flex items-center gap-2 text-emerald-600 hover:underline hover:text-emerald-800 text-base transition-colors"
        >
          <ArrowLeft className="w-5 h-5" />
          Back to Jobs
        </button>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight transition-colors duration-300 hover:text-emerald-700">
            {job.title}
          </h1>
          <span
        ...
        <div className="flex flex-wrap gap-4 text-gray-600 text-base mb-6">
          {job.location && (
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-100 text-gray-700 font-medium transition-all hover:bg-emerald-50 hover:text-emerald-700">
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
          ...
          {job.type && (
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-100 text-gray-700 font-medium transition-all hover:bg-emerald-50 hover:text-emerald-700 capitalize">
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
          ...
          <div className="mb-5">
            ...
            <div className="flex flex-wrap gap-2">
              {job.tags.map((tag) => (
                <span
                  key={tag}
                  className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 hover:bg-emerald-200"
                >
                  {tag}
                </span>
              ))}
